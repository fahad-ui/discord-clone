...
export const ServerSideBar = async({serverId}:ServerSideBarProps) => {
    ...
    const server = await db.server.findUnique({
        ...
        include:{
            ...
            members:{
                include:{
                    profile:true //we are going to load image, name , profile etc
                },
                orderBy:{
                    role:"asc"
                }
    ...
    const role = server?.members.find((member)=> member.profileId === profile.id)?.role //extracts the current profile's role
                    ...
                    <ServerSearch
                        data={[
                            ...
                            },
                            {
                                label:"Members",
                                type:"member",
                                data: members?.map((member)=>({ id:member.id, name:member.profile.name, icon:roleIconMap[member.role]}))
                            }
                        ]}
                    />
                </div>
